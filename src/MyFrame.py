...
from actions import repo_search, load_repos, save_repos
...
class MyFrame(wx.Frame):
    ...
    def on_exit(self, event: wx.CloseEvent) -> None:
        ...
        if not self.exit_task_running:
            ...
            # Schedule async task
            run_async_task(self.final_save())
    ...
    def create_menu(self):
        ...
        AsyncBind(wx.EVT_MENU, self.load, self, load_item)
        AsyncBind(wx.EVT_MENU, self.save, self, save_item)

    async def load(self, event: wx.MenuEvent | None = None):
        self.SetStatusText("Loading...")
        str_to_ctrl(await load_repos(), self.list_all_repos)
        self.SetStatusText("Ready")

    async def save(self, event: wx.MenuEvent | None = None):
        await save_repos(ctrl_to_str(self.list_all_repos))
